...
def get_logo_config(report_type: str = 'default') -> Dict:
    ...
    return DEFAULT_LOGO_CONFIGS.get(report_type, DEFAULT_LOGO_CONFIGS['provider_payment'])
...
def get_logo_path(report_type: str = 'default') -> Optional[str]:
    ...
    """
    config = get_logo_config(report_type)
    logo_path = config.get('path')
    
    if logo_path and validate_logo_path(logo_path):
        return logo_path
...
def resize_logo_for_pdf(logo_path: str, max_width: int = 50, max_height: int = 25) -> Tuple[float, float]:
    ...
    """
    dimensions = get_logo_dimensions(logo_path)
    if not dimensions:
...
class LogoManager:
    """Compatibility wrapper for old LogoManager usage"""
    DEFAULT_LOGOS = DEFAULT_LOGO_CONFIGS  # Backward compatibility
    
    @staticmethod
    def get_logo_config(report_type: str = 'default') -> Dict:
        return get_logo_config(report_type)
    
    @staticmethod
    def validate_logo_path(logo_path: str) -> bool:
        return validate_logo_path(logo_path)
    
    @staticmethod
    def get_logo_path(report_type: str = 'default') -> Optional[str]:
        return get_logo_path(report_type)
    
    @staticmethod
    def create_logo_directory() -> str:
        return create_logo_directory()
    
    @staticmethod
    def list_available_logos() -> list:
        return list_available_logos()
    
    @staticmethod
    def get_logo_dimensions(logo_path: str) -> Optional[Tuple[int, int]]:
        return get_logo_dimensions(logo_path)
    
    @staticmethod
    def resize_logo_for_pdf(logo_path: str, max_width: int = 50, max_height: int = 25) -> Tuple[float, float]:
        return resize_logo_for_pdf(logo_path, max_width, max_height) 